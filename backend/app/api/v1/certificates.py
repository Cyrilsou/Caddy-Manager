...
async def list_certificates(db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)):
...
async def get_certificate(cert_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)):
...
@router.post("/refresh")
async def refresh_certificates(_: User = Depends(get_current_user)):
